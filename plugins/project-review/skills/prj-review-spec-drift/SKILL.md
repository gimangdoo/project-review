...
## L01(plan-adherence) 와의 경계
- **L01** = plan/roadmap/intent_profile.workflow 의 **행동 enum** (해야 할/하지 말 것) vs 실행 (전향)
- **L06 (본 lane)** = README·CLAUDE.md·주석·docstring 의 **사실 진술** (X 함수는 Y 반환, env Z 필수 등) vs 코드 사실 (후향)
- **intent_profile.md = L01 우선권**. 본 lane은 intent_profile 직접 참조 회피 — README·CLAUDE.md·코드 주석·docstring·티켓 본문 한정
- 동시 활성 시 L01 emit 한 finding은 본 lane skip — 중복 회피
...
## 출력 박제
`{review_out}/{date}_{slug}/L06_spec-drift.md`
