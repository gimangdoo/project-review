...
3. **메모리**
   ...
   - 클로저로 큰 객체 보유
   - stream 미사용 대용량 read
4. **LLM 비용** (provider-agnostic)
   - prompt 길이 증가 (system + user)
   - prompt caching 미사용:
     - Anthropic SDK → `cache_control` 옵션 부재
     - OpenAI → system message 재사용 패턴 부재 (자동 prefix 캐싱 깨짐)
     - Gemini → context caching API (`cachedContents`) 미사용
     - 로컬 LLM (llama.cpp/vLLM) → KV cache reuse 미설정
...
## 출력 박제
`{review_out}/{date}_{slug}/L09_perf-cost.md`
